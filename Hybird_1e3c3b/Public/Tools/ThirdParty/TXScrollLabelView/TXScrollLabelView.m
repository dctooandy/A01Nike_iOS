...
- (void)setScrollVelocity:(NSTimeInterval)scrollVelocity {
    ...
    if (_scrollType == TXScrollLabelViewTypeLeftRight || _scrollType == TXScrollLabelViewTypeUpDown || _scrollType == TXScrollLabelViewNoRepeatTypeLeftRight) {
...
- (void)setupInitial {
    switch (_scrollType) {
        ...
        case TXScrollLabelViewNoRepeatTypeLeftRight:
...
- (void)setupSubviewsLayout_LeftRight {
    ...
    [self setupLRUDTypeLayoutWithMaxSize:CGSizeMake(labelMaxW, labelMaxH) width:labelW height:labelH completedHandler:^(CGSize size) {
        ...
        if (_scrollType != TXScrollLabelViewNoRepeatTypeLeftRight) {
...
- (void)updateScrollingType_LeftRight {
    ...
    if (_scrollType == TXScrollLabelViewNoRepeatTypeLeftRight && self.contentOffset.x >= (_scrollInset.left + self.upLabel.tx_width + self.scrollSpace)) {
        [self endup];
        if (self.endScrollBlock) {
            self.endScrollBlock();
        }
    } else if (self.contentOffset.x >= (_scrollInset.left + self.upLabel.tx_width + self.scrollSpace)) {
        /** 更新 Label.text */
