...
+ (NSArray *)gateways {
    return [AppdelegateManager shareManager].gateways;
...
+ (NSString *)defaultH5Domain {
    return [AppdelegateManager shareManager].websides.firstObject;
//    switch (EnvirmentType) {
//        case 0:
...
+ (NSArray *)websides
{
    return [AppdelegateManager shareManager].websides;
}
+ (NSString *)defaultCDN {
    switch (EnvirmentType) {
        case 0:
