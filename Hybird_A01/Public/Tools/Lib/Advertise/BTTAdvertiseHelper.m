...
@implementation BTTAdvertiseHelper

static BTTAdvertiseHelper* _instance = nil;
...
+ (void)showAdvertiserView:(NSArray<NSString *> *)imageArray {
    ...
    NSString *filePath = [[BTTAdvertiseHelper sharedInstance] getFilePathWithImageName:[NSUserDefaults.standardUserDefaults valueForKey:adImageName]];
    
    BOOL isExist = [[BTTAdvertiseHelper sharedInstance] isFileExistWithFilePath:filePath];
    if (isExist) {// 图片存在
        BTTAdvertiseView *advertiseView = [[BTTAdvertiseView alloc] initWithFrame:UIScreen.mainScreen.bounds];
        advertiseView.filePath = filePath;
        [advertiseView show];
    }
    
    // 2.无论沙盒中是否存在广告图片，都需要重新调用广告接口，判断广告是否更新
    [[BTTAdvertiseHelper sharedInstance] getAdvertisingImage:imageArray];
}
