...
+ (instancetype)shareInstance {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:instance selector:@selector(finishLoginGame:) name:@"FinishLoginGame" object:nil];
    });
    return instance;
}
...
- (void)configProduct:(NSString *)productID {
    // 3S项目 启动埋点
    [self startRecordTime:CNEventAppLaunch];
    [IN3SAnalytics configureSDKWithProduct:productID];
}
...
- (void)recordAGQJTime:(BOOL)isPreload {
    ...
    // 加载即上报数据
    [IN3SAnalytics loadAGQJWithResponseTime:0 loadFinish:NO msg:@"" timestamp:timeString];
}
...
- (void)finishLoginGame:(NSNotification *)notification {
